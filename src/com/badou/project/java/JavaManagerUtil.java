package com.badou.project.java;
...
public class JavaManagerUtil {
	...
	public static List<String> getJavaProperties(CompilationUnit cu) {
		...
		System.out.println("获取到FieldDeclaration:" + fieldDeclarations);
		for (FieldDeclaration filed : fieldDeclarations) {
			System.out.println("添加filed:" + filed);
	 ...
	 * @throws Exception
	 */
	public static boolean isInterface(String javasrc) throws Exception {
		try {
			File file = new File(javasrc);
			CompilationUnit cu = JavaParser.parse(file);
